class Rule:
    ...
    def __init__(self, rule_id: int, rule_name: str, rule_condition: callable, rule_outcome: str):
        # validations
        if rule_id is None or type(rule_id) is not int or rule_id < 0:
            raise ValueError("Rule id is required.")
        if rule_name is None or type(rule_name) is not str or not rule_name.strip():
            raise ValueError("Rule name is required.")
        if rule_condition is None or not callable(rule_condition):
            raise ValueError("Rule condition is required.")
        if rule_outcome is None or type(rule_outcome) is not str or not rule_outcome.strip():
